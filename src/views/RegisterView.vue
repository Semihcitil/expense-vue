    ...
    <div class="auth-container">
      ...
      <div class="auth-card">
        <div class="auth-card-header">
          ...
          <p class="auth-desc">Giderlerinizi takip etmeye başlayın</p>
        </div>
        ...
        <form @submit.prevent="handleRegister" class="auth-form">
          <AppInput v-model="form.name" label="Ad Soyad" type="text" placeholder="Adınız Soyadınız" required />
          <AppInput v-model="form.email" label="E-posta" type="email" placeholder="ornek@email.com" required />
          <AppInput v-model="form.password" label="Şifre" type="password" placeholder="En az 6 karakter" required />
          <AppButton type="submit" :loading="loading" full size="lg">Kayıt Ol →</AppButton>
        </form>

        <div class="auth-footer">
          Zaten hesabınız var mı?
          <RouterLink to="/login" class="auth-link">Giriş yapın</RouterLink>
        </div>
      </div>
    </div>
  </div>
...
const handleRegister = async () => {
  error.value = ''
  if (form.value.password.length < 6) { error.value = 'Şifre en az 6 karakter olmalı'; return }
  loading.value = true
  try {
...
.auth-card {
  width: 100%; background: var(--bg-elevated); border: 1px solid var(--border-med);
...
.auth-link { color: var(--accent-light); font-weight: 600; margin-left: 4px; transition: color var(--t); }
